...
public class Municipio {
    ...
    public int insertar() {
        ...
        try {
            java.sql.PreparedStatement ps = ConexionRRHHBD.getConnection().prepareStatement("insert into "
                    + "catmunicipio(id_municipio,descripcion)values(?,?)");
            ps.setInt(1, id);
 ...
 public int actualizar() {
        int cont = 0;
        try {
            java.sql.PreparedStatement ps = ConexionRRHHBD.getConnection().prepareStatement("update catmunicipio "
                    + "set descripcion=? "
                    + "where id_municipio =?");
 ...
 public int eliminar() {
        int cont = 0;
        try {
            java.sql.PreparedStatement ps = ConexionRRHHBD.getConnection().prepareStatement("delete from catmunicipio "
                    + "where id_municipio=?");
